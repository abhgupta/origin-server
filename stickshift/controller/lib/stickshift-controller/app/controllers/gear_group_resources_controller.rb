class GearGroupResourcesController < BaseController
  ...
  def index
    ...
    if app.nil?
      ...
      if selected_gear_group.nil?
        ...
        @reply = RestReply.new(:not_found)
        message = Message.new(:error, "Gear group not found.", 159)
        @reply.messages.push(message)
        respond_with @reply, :status => @reply.status
      else
        storage = selected_gear_group.addtl_fs_gb.to_s.empty? ? "1" : selected_gear_group.addtl_fs_gb
        @reply = RestReply.new(:ok, "resources", RestGearGroupResources.new(selected_gear_group.uuid, storage))
  ...
  def update
    ...
    if max_storage.nil?
      ...
      @reply = RestReply.new(:not_found)
      message = Message.new(:error, "User is not allowed to change storage quota", 160)
      @reply.messages.push(message)
    ...
    begin 
      ...
      @reply = RestReply.new(:not_found)
      message = Message.new(:error, "Invalid storage value provided", 161)
      @reply.messages.push(message)
      respond_with(@reply) do |format|
        format.xml { render :xml => @reply, :status => @reply.status }
        format.json { render :json => @reply, :status => @reply.status }
    ...
    if num_storage < 1 or num_storage > max_storage
      log_action(@request_id, @cloud_user.uuid, @cloud_user.login, "UPDATE_GEAR_GROUP_RESOURCES", false, "User is not allowed to change storage quota")
      @reply = RestReply.new(:not_found)
      message = Message.new(:error, "Storage value must be between 1 and #{max_storage}", 162)
      @reply.messages.push(message)
      respond_with(@reply) do |format|
        format.xml { render :xml => @reply, :status => @reply.status }
        format.json { render :json => @reply, :status => @reply.status }
    ...
    if app.nil?
      ...
      if selected_gear_group.nil?
        ...
        @reply = RestReply.new(:ok, "resources", RestGearGroupResources.new(selected_gear_group.uuid, selected_gear_group.addtl_fs_gb))
